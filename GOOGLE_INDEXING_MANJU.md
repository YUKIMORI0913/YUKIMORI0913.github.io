...
## 所有権確認タグの重複（2026-09-23 記録）

`manju.unagitani.com` と `corporate.unagitani.com` の `<head>` に、同じ
`google-site-verification` の値が入っています。HTMLタグ方式のトークンは
プロパティごとに発行されるため、同じ値を別ホストへ貼っても所有権は確認
できません。コーポレートサイト側のプロパティが未確認で、サイトマップ送信も
インデックス登録リクエストも届いていない可能性があります。

`unagitani.com` のドメインプロパティをDNSのTXTレコードで確認すると、両サブ
ドメインを一度にカバーできます。お名前.comでの具体的な手順は、コーポレート
サイトのリポジトリの `docs/SEARCH_INDEXING.md` にまとめています。ドメイン
プロパティの確認が完了するまで、既存の確認用metaタグは削除しないでください。
